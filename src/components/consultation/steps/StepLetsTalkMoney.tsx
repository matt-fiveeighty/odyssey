...
function getBudgetInsight(pointBudget: number): string {
  ...
  return `At $${Math.round(pointBudget).toLocaleString()}/yr, you can comfortably maintain ${stateCount} states in your point portfolio. ${BUDGET_TIERS[tier]}`;
}
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex items-center gap-4 mb-2">
              ...
              <span className="text-2xl font-bold text-primary w-28 text-right">${Math.round(wizard.pointYearBudget).toLocaleString()}</span>
            </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mb-2">
              ...
              <span className="text-2xl font-bold text-chart-2 w-28 text-right">${Math.round(wizard.huntYearBudget).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-[10px] text-muted-foreground">
        ...
        <div className="p-3 rounded-xl bg-secondary/30 border border-border/50">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
            <div>
              ...
              <span className="font-bold text-foreground ml-1">${Math.round(wizard.pointYearBudget * 10).toLocaleString()}</span>
            </div>
            <div>
              <span className="text-muted-foreground/60">With 3 hunt years:</span>
              <span className="font-bold text-foreground ml-1">${Math.round(wizard.pointYearBudget * 7 + wizard.huntYearBudget * 3).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </CardContent>
