...
export function SubscribeCalendar({ assessment }: SubscribeCalendarProps) {
  ...
  // ── Render: ready (expanded panel) ───────────────────────────────────────

  if (status === "ready" && subscription) {
    return (
      <div className="bg-secondary/30 border border-border rounded-xl p-4 space-y-3 max-w-[280px]">
        {/* Header */}
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            aria-label="Collapse subscription panel"
          >
            &times;
          </button>
        </div>
