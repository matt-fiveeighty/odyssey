...
<task type="tdd">
  <name>Task 1: VerifiedDatum type system and provenance helpers</name>
  ...
  <action>
    ...
    RED: Write failing tests in src/lib/engine/verified-datum.test.ts covering all test cases from behavior section. Run tests — they MUST fail (no implementation yet).

    GREEN: Create src/lib/engine/verified-datum.ts with:
       - DataConfidence type union
       - VerifiedDatum&lt;T&gt; interface
       - CONFIDENCE_ORDER constant: { verified: 0, user_reported: 1, estimated: 2, stale: 3 }
       - STALE_THRESHOLDS constant object with default and category-specific thresholds
    ...
    Use Vitest for tests (check if vitest is in devDependencies; if not, use Jest patterns matching existing test setup).
  ...
  <done>
    - VerifiedDatum&lt;T&gt; type exported and usable with any value type
    - All factory/helper functions (verified, estimated, userReported, unwrap, verifyBatch, deriveConfidence) exported and tested
