/**
 * South Dakota (GFP) Draw Data Scraper
 *
 * South Dakota Game Fish & Parks provides draw statistics through their
 * website and downloadable draw reports.
 *
 * Data sources:
 *   - Draw info: https://gfp.sd.gov/draw-information/
 *   - Draw results: https://gfp.sd.gov/draw-results/
 *   - Hunting seasons: https://gfp.sd.gov/hunting/
 *   - Fees: https://gfp.sd.gov/licenses-permits/fees/
 *   - Big game: https://gfp.sd.gov/big-game/
 *
 * SD draw system:
 *   - Preference point system for most big game
 *   - Limited tags by unit for elk, bighorn sheep, mountain goat
 *   - "Any deer" vs "antlerless" permit distinction
 *   - Custer State Park has special hunts
 *   - Prairie dog and predator are separate
 *
 * Species available:
 *   elk, mule_deer, whitetail, pronghorn, bighorn_sheep, mountain_goat,
 *   mountain_lion
 */
...
const GFP_BASE = "https://gfp.sd.gov";
const GFP_DRAW_INFO = `${GFP_BASE}/draw-information/`;
const GFP_DRAW_RESULTS = `${GFP_BASE}/draw-results/`;
const GFP_BIG_GAME = `${GFP_BASE}/big-game/`;
const GFP_HUNTING = `${GFP_BASE}/hunting/`;
const GFP_FEES = `${GFP_BASE}/licenses-permits/fees/`;

/** Species available in SD draws */
const SD_SPECIES: { name: string; speciesId: string; keywords: string[] }[] = [
  ...
  { name: "Mountain Lion", speciesId: "mountain_lion", keywords: ["mountain lion", "lion", "cougar"] },
];

const QUERY_YEARS = [2025, 2024, 2023];

// ---------------------------------------------------------------------------
// South Dakota scraper
// ---------------------------------------------------------------------------

export class SouthDakotaScraper extends BaseScraper {
  stateId = "SD";
  stateName = "South Dakota";
  sourceUrl = GFP_DRAW_INFO;
  ...
  async scrapeUnits(): Promise<ScrapedUnit[]> {
    ...
    try {
      this.log("Scraping SD GFP for hunting units...");

      // Strategy 1: Parse draw info page
      const html = await this.fetchPage(GFP_DRAW_INFO);
      ...
      // Strategy 2: Try big game page
      try {
        const bgHtml = await this.fetchPage(GFP_BIG_GAME);
      ...
      // Strategy 3: Draw results page
      try {
        const drawHtml = await this.fetchPage(GFP_DRAW_RESULTS);
    ...
    this.log(`Extracted ${units.length} SD units`);
    return units;
  }
  ...
  async scrapeDrawHistory(): Promise<ScrapedDrawHistory[]> {
    ...
    try {
      this.log("Scraping SD GFP draw results...");

      // Strategy 1: Parse draw results page for tables
      const html = await this.fetchPage(GFP_DRAW_RESULTS);
      const tableResults = this.parseDrawFromHtml(html);
      results.push(...tableResults);

      // Strategy 2: Try draw info page
      if (results.length === 0) {
        const infoHtml = await this.fetchPage(GFP_DRAW_INFO);
        const infoResults = this.parseDrawFromHtml(infoHtml);
      ...
      if (results.length === 0) {
        ...
        const allHtml = await this.fetchPage(GFP_DRAW_RESULTS);
    ...
    this.log(`Total SD draw history rows: ${results.length}`);
    return results;
  }
  ...
  async scrapeDeadlines(): Promise<ScrapedDeadline[]> {
    ...
    try {
      this.log("Scraping SD GFP deadlines...");
      const urls = [GFP_DRAW_INFO, GFP_HUNTING, GFP_BIG_GAME];
    ...
    this.log(`Found ${deadlines.length} SD deadlines`);
    return deadlines;
  }
  ...
  async scrapeSeasons(): Promise<ScrapedSeason[]> {
    ...
    try {
      this.log("Scraping SD GFP season dates...");
      const html = await this.fetchPage(GFP_HUNTING);

      const seasonPattern = /(archery|muzzleloader|rifle|firearm|general)[^:]*?:\s*((?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\.?\s+\d{1,2})\s*[-–]\s*((?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\.?\s+\d{1,2})/gi;
      let match: RegExpExecArray | null;
      ...
      while ((match = seasonPattern.exec(html)) !== null) {
        ...
        for (const speciesId of speciesIds) {
          seasons.push({ stateId: "SD", speciesId, seasonType, startDate, endDate, year });
        }
      }
    } catch (err) {
    ...
    this.log(`Found ${seasons.length} SD season entries`);
    return seasons;
  }
  ...
  async scrapeRegulations(): Promise<ScrapedRegulation[]> {
    ...
    try {
      this.log("Scraping SD GFP regulation updates...");
      const url = GFP_HUNTING;
      const html = await this.fetchPage(url);
    ...
    this.log(`Found ${regs.length} SD regulation entries`);
    return regs;
  }
  ...
  async scrapeLeftoverTags(): Promise<ScrapedLeftoverTag[]> {
    ...
    try {
      this.log("Checking SD GFP for leftover tags...");
      const urls = [GFP_DRAW_RESULTS, GFP_DRAW_INFO];
    ...
    this.log(`Found ${leftovers.length} SD leftover tags`);
    return leftovers;
  }
  ...
  private parseUnitsFromHtml(html: string): ScrapedUnit[] {
    ...
    for (const table of tables) {
      ...
      for (const tr of trs) {
        ...
        const unitCode = row["unit"] || row["area"] || row["permit"] || row["season"] || cells[0] || "";
  ...
  private parseDrawFromHtml(html: string): ScrapedDrawHistory[] {
    ...
    for (const table of tables) {
      ...
      for (const tr of trs) {
        ...
        const unitCode = row["unit"] || row["area"] || row["season"] || cells[0] || "";
        ...
        if (applicants > 0 || tags > 0) {
          results.push({ unitId: `SD:${speciesId}:${unitCode}`, year, applicants, tags, odds, minPointsDrawn: null });
        }
      }
    }
  ...
  private parseCsvDrawData(rows: string[][]): ScrapedDrawHistory[] {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const unitCode = row["unit"] || row["area"] || row["season"] || "";
      if (!unitCode) continue;
  ...
  private extractDownloadLinks(html: string, extensions: string[]): string[] {
    ...
    while ((match = pattern.exec(html)) !== null) {
      ...
      if (extensions.some((ext) => href.toLowerCase().endsWith(ext))) {
        links.push(href.startsWith("http") ? href : `${GFP_BASE}${href}`);
      }
    }
    return links;
  }
  ...
  private extractDeadlinesFromHtml(html: string, year: number): ScrapedDeadline[] {
    ...
    for (const pattern of datePatterns) {
      ...
      while ((match = pattern.exec(html)) !== null) {
        ...
        const speciesIds = this.detectSpeciesFromContext(context);
        for (const speciesId of speciesIds) {
          deadlines.push({ stateId: "SD", speciesId, deadlineType, date: dateStr, year, notes: context.trim().substring(0, 200) });
        }
      }
    }
      ...
      for (const tr of trs) {
        ...
        for (const cell of cells) {
          ...
          if (dollarMatch) {
            ...
            if (amt > 0 && feeName) {
              fees.push({
                stateId: "SD", feeName: feeName.substring(0, 100), amount: amt,
  ...
  private extractRegulationsFromHtml(html: string, sourceUrl: string): ScrapedRegulation[] {
    ...
    while ((match = newsPattern.exec(html)) !== null) {
      ...
      if (
        ...
        regs.push({ stateId: "SD", title: text.substring(0, 200), summary: text, sourceUrl, category });
      }
    }
    ...
    for (const table of tables) {
      ...
      for (const tr of trs) {
        ...
        if (unitCode && available > 0) {
          leftovers.push({ stateId: "SD", speciesId, unitCode, tagsAvailable: available, sourceUrl });
        }
      }
    }
